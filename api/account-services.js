    ...
    if (req.method === 'POST') {
      try {
        ...
        if (cardAction === 'setChannelLock') {
          ...
          } else if (column === 'atm_enabled') {
            await sql`UPDATE credit_card_details SET atm_enabled = ${enabledBool} WHERE account_id = ${account.id}`;
          } else {
            await sql`UPDATE credit_card_details SET international_enabled = ${enabledBool} WHERE account_id = ${account.id}`;
          }

          const label = channel === 'online' ? 'Online purchases'
            : channel === 'atm' ? 'ATM withdrawals'
            : 'International transactions';

          await createNotification(
            session.userId,
            enabledBool ? `${label} Enabled` : `${label} Disabled`,
            enabledBool
              ? `${label} have been enabled on your credit card.`
              : `${label} have been disabled on your credit card.`
          );

          return res.status(200).json({ success: true, channel, enabled: enabledBool });
        }
