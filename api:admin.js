...
function generateAccountHistory({ accountType, yearsBack, density }) {
  ...
  // Date generation is clamped so every iteration reliably adds a row
  // (avoids continue-skips that could theoretically exhaust the guard
  // without reaching the floor).
  const MIN_TRANSACTIONS = 50;
  const rangeMs = Math.max(1, now.getTime() - start.getTime());
  let topUpGuard = 0;
  while (rows.length < MIN_TRANSACTIONS && topUpGuard < 2000) {
    topUpGuard++;
    const cat = pick(WEIGHTED_POOL);
    let date = new Date(start.getTime() + Math.random() * rangeMs);
    // Clamp base date into [start, now]
    if (date < start) date = new Date(start);
    if (date > now) date = new Date(now);
    date.setHours(randomInt(6, 22), randomInt(0, 59), randomInt(0, 59), 0);
    // Re-clamp after applying time-of-day (setHours can push past "now")
    if (date > now) {
      date = new Date(now.getTime() - randomInt(0, 3_600_000)); // up to 1 hour before now
    }
    if (date < start) date = new Date(start);
    const merchant = pick(cat.merchants);
